...
set -x
peer channel fetch 0 $BLOCKFILE -o 192.168.8.200:7050 --ordererTLSHostnameOverride orderer.example.com -c $CHANNEL_NAME --tls --cafile "$ORDERER_CA" >&log.txt # CVS: ORDERER:7050 
res=$?
{ set +x; } 2>/dev/null
cat log.txt
